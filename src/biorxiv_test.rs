    ...
    for b in bodies {
        match b {
            Ok(b) => {
                    ...
                    match xml_parser_one_string_creators.find("., ") {
                        Some(end_of_creator) => {
                            biorxiv_page_struct.items[count]
                                .creators
                                .push(xml_parser_one_string_creators[..end_of_creator].to_string());
                ...
                println!("items in parsed {}", biorvix_struct.items.len());
                println!("items in used {}", biorvix_struct.items.len());
                biorxiv_structs_vec.insert(vec_of_keys[key_count].to_string(), biorxiv_page_struct);
            }
            Err(e) => {
