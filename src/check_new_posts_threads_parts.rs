...
pub async fn check_new_posts_threads_parts() -> Option<(
    ...
    if !ProviderKind::get_enabled_string_name_vec().is_empty() {
        ...
        let providers_link_parts = ProviderKind::get_providers_json_local_data();
        if !providers_link_parts.is_empty() {
            ...
            for provider_name in ProviderKind::get_enabled_providers_vec() {
                match config_provider_string_to_enum_struct_hashmap
                    .get(ProviderKind::get_string_name(provider_name))
                {
                                ...
                                match providers_link_parts.get(&provider_name) {
                                    Some(arxiv_link_parts) => {
                                        if arxiv_link_parts.is_empty() {
                                            ...
                                            let provider_kind_handle_clone = *provider_kind_handle;
                                ...
                                match providers_link_parts.get(&provider_name) {
                                    Some(biorxiv_link_parts) => {
                                        if biorxiv_link_parts.is_empty() {
                                            ...
                                            if CONFIG.enable_providers_prints.enable_prints_biorxiv
                                            {
                                                println!(
                                                    "{:#?} elements in {:#?} HashMap",
                                            ...
                                            let provider_kind_handle_clone = *provider_kind_handle;
                                            let vec_of_provider_links =
                                                generate_biorxiv_links(biorxiv_link_parts.to_vec());
                                            threads_vec_checker.push(true);
                                            threads_vec.push(thread::spawn(move || {
                                                providers_new_posts_check(
                                                    provider_kind_handle_clone,
                                ...
                                match providers_link_parts.get(&provider_name) {
                                    Some(github_link_parts) => {
                                        if github_link_parts.is_empty() {
                                            ...
                                            if CONFIG.enable_providers_prints.enable_prints_github {
                                                println!(
                                                    "{:#?} elements in {:#?} HashMap",
                                            ...
                                            let provider_kind_handle_clone = *provider_kind_handle;
                                            let vec_of_provider_links =
                                                generate_github_links(github_link_parts.to_vec());
                                            threads_vec_checker.push(true);
                                            threads_vec.push(thread::spawn(move || {
                                                providers_new_posts_check(
                                                    provider_kind_handle_clone,
                                ...
                                match providers_link_parts.get(&provider_name) {
                                    Some(medrxiv_link_parts) => {
                                        if medrxiv_link_parts.is_empty() {
                                            ...
                                            if CONFIG.enable_providers_prints.enable_prints_medrxiv
                                            {
                                                println!(
                                                    "{:#?} elements in {:#?} HashMap",
                                            ...
                                            let provider_kind_handle_clone = *provider_kind_handle;
                                            let vec_of_provider_links =
                                                generate_medrxiv_links(medrxiv_link_parts.to_vec());
                                            threads_vec_checker.push(true);
                                            threads_vec.push(thread::spawn(move || {
                                                providers_new_posts_check(
                                                    provider_kind_handle_clone,
                                ...
                                match providers_link_parts.get(&provider_name) {
                                    Some(reddit_link_parts) => {
                                        if reddit_link_parts.is_empty() {
                                            ...
                                            if CONFIG.enable_providers_prints.enable_prints_reddit {
                                                println!(
                                                    "{:#?} elements in {:#?} HashMap",
                                            ...
                                            let provider_kind_handle_clone = *provider_kind_handle;
                                            let vec_of_provider_links =
                                                generate_reddit_links(reddit_link_parts.to_vec());
                                            threads_vec_checker.push(true);
                                            threads_vec.push(thread::spawn(move || {
                                                providers_new_posts_check(
                                                    provider_kind_handle_clone,
