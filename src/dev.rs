...
> {
    ...
    let auth_header = if let Some(auth_header) = auth_header {
        ...
        println!("2");
        let error_with_serialize_deserialize = tufa_common::server::extractors::project_commit_extractor::ProjectCommitExtractorCheckErrorNamed::ProjectCommitExtractorNotEqual {
                    project_commit_not_equal: "different project commit provided, services must work only with equal project commits",
                    project_commit_to_use: tufa_common::common::git::get_git_commit_link::GetGitCommitLink::get_git_commit_link(&tufa_common::global_variables::compile_time::project_git_info::PROJECT_GIT_INFO),
                    code_occurence: tufa_common::code_occurence!(),
                }.into_serialize_deserialize_version();
        // serde_json::to_string(&error_with_serialize_deserialize).unwrap_or_else(|_|{
        ...
        // });
        println!("2");
        // Err(Request::from_parts(parts, Body::from(bytes)))
        // Err(axum::http::StatusCode::BAD_REQUEST)
        return Err((
            axum::http::StatusCode::UNAUTHORIZED,
            axum::Json(error_with_serialize_deserialize),
        )
            .into_response());
...
async fn handler(// extract the current user, set by the middleware
...
//     // (
//     //     axum::http::StatusCode,
//     //     axum::Json<tufa_common::server::extractors::project_commit_extractor::ProjectCommitExtractorCheckErrorNamedWithSerializeDeserialize>,
//     // ),
// > {
...
//     } else {
//         println!("3");
//         let error_with_serialize_deserialize = tufa_common::server::extractors::project_commit_extractor::ProjectCommitExtractorCheckErrorNamed::NoProjectCommitExtractorHeader {
