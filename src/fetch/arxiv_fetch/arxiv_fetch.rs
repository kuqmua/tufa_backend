...
use crate::fetch::arxiv_fetch::arxiv_fetch_and_parse_xml::arxiv_fetch_and_parse_xml;
...
pub fn arxiv_part() -> bool {
    if reach_provider(ARXIV_URL.to_string()) {
        ...
        if ENABLE_PRINTS_ARXIV {println!("{:#?} elements in Arxiv HashMap", arxiv_links_in_hash_map.len())};
        ...
        arxiv_fetch_and_parse_xml(vec_of_links, vec_of_keys);//тут есть возвращаемое значение let vec_of_vec_of_strings = 
