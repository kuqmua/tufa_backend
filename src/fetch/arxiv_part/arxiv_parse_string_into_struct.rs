...
pub fn arxiv_parse_string_into_struct(
    ...
    let mut arxiv_post_struct_handle: RxivPostStruct = RxivPostStruct::new();
    let are_there_items_handle: AreThereItems; // = AreThereItems::Initialized
    match fetch_tuple_result.find("</item>") {
        Some(_) => {
            let arxiv_struct_from_str_result: Result<XmlRxivParserStruct, serde_xml_rs::Error> =
                from_str(&fetch_tuple_result);
            match arxiv_struct_from_str_result {
                Ok(arxiv_struct) => {
                    let mut count = 0;
                    let mut arxiv_page_struct: RxivPostStruct = RxivPostStruct::new();
                    loop {
                        if count < arxiv_struct.items.len() {
                            let mut arxiv_post: RxivPost = RxivPost::new();
