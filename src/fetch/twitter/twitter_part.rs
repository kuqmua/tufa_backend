...
pub fn twitter_part(
    ...
    if !twitter_available_providers_links.is_empty() {
        ...
        let links_for_each_provider: usize = links.len() / twitter_available_providers_links.len();
        ...
        for element in vec_of_links {
            if counter < even_links_length_size_for_remaind {
                ...
                vec_of_hashmap_parts[vec_of_hashmap_parts_position].insert(element.0, element.1);
            }
            counter += 1;
        }
        ///
