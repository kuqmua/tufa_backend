...
pub fn get_twitter_links(twitter_provider_names: Vec<&str>) -> HashMap<&'static str, String> {
    ...
    println!("how_many_links_on_diff_provider {} subs_nicknames.len() {} twitter_provider_names.len() {}", how_many_links_on_diff_provider, subs_nicknames.len(), twitter_provider_names.len() );
    // let subslen = subs_nicknames.len();
    let mut twitter_sections_links: HashMap<&str, String> =
        HashMap::with_capacity(subs_nicknames.len());
    let mut i = 0;
    let mut m = 0;
    println!("twitter_provider_names {:#?}", twitter_provider_names);
    for sub_nickname in subs_nicknames {
        ...
        if m != 0 && m % (how_many_links_on_diff_provider + 1) == 0 {
            println!("+1");
            i += 1;
        }
        m += 1;
