                    ...
                    match fetch_head_content.find(end_line_to_find) {
                        ...
                        Some(index) => {
                            ...
                            match first_line.find(first_slice_to_find) {
                                None => {
                                    branch = format!("error: no '{}' inside '{}'", first_slice_to_find, first_line);
                                    repo_link = format!("error: no '{}' inside '{}'", first_slice_to_find, first_line);
                                },
                                Some(first_branch_index) => {
                                    match first_line.find(second_slice_to_find) {
                                        None => {
                                            branch = format!("error: no '{}' inside '{}'", second_slice_to_find, first_line);
                                            repo_link = format!("error: no '{}' inside '{}'", second_slice_to_find, first_line);
                                        },
                                        Some(second_branch_index) => {
                                            branch = first_line[first_branch_index + first_slice_to_find.len()..second_branch_index].to_string();
                                            repo_link = first_line[second_branch_index + second_slice_to_find.len()..].replace(':', "/");
                                        },
                                    }
                                },
                            }
