...
use crate::helpers::http_request::request_builder_methods::content_length::sync_http_request_content_length::sync_http_request_content_length;
...
pub async fn sync_http_request_content_length_wrapper<
    //client generics
...
>(
    ...
    should_trace: bool,
) -> Result<Option<u64>, Box<HttpRequestWrapperContentLengthError>>
where
    ...
    BearerAuthGeneric: std::fmt::Display,
{
    ...
    .await
    {
        ...
        Ok(request_builder) => match sync_http_request_content_length(request_builder, false) {
            Err(e) => Err(Box::new(
                HttpRequestWrapperContentLengthError::init_error_with_possible_trace(
