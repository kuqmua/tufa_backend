...
pub async fn init_mongo(providers_json_local_data_hashmap: HashMap<ProviderKind, Vec<String>>) -> Result<(), MongoInitDbError> {
    let client_options_result = ClientOptions::parse(&mongo_get_db_url()).await;
    match client_options_result {
        Err(e) => {
            return Err(MongoInitDbError::ClientOptionsParse(e));
        },
        Ok(client_options) => {
                ...
                Ok(client) => {
                    ...
                    for (pk, result) in result_vec {
                        match result {//todo filter
                            Err(e) => return Err(MongoInitDbError::CollectionCountDocuments((pk, e))),
                            Ok(documents_number) => {
                                if documents_number > 0 {
                                    return Err(MongoInitDbError::CollectionIsNotEmpty((pk, documents_number)));
                                }
                            },
                        }
                    }
                    ...
                    for (pk, result) in result_vec {
                        match result {
                            Err(e) => return Err(MongoInitDbError::CollectionInsertMany((pk, e))),//loosing error info?
                            Ok(_) => (),
                        }
                    }
                    Ok(())
