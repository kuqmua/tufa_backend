    ...
    while let Some(document) = cursor.try_next().await? {
        if let Some(bson_handle) = document.get(db_collection_document_field_name_handle) {
            match bson_handle {
                ...
                other_bson_type => { return Err(MongoGetDocumentsAsStringVectorError {
                    source: Box::new(MongoGetDocumentsAsStringVectorErrorEnum::WrongBsonType(other_bson_type.clone()))
                });},
            }
        }
    }
