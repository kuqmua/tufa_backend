pub async fn mongo_insert_data<'a>(
...
) -> Result<(), Box<tufa_common::repositories_types::tufa_server::mongo_integration::mongo_insert_data::MongoInsertDataErrorNamed<'a>>> {
    ...
    .filter_map(|(pk, result)| {
        if let Err(e) = result {
            return Some((
                pk.to_string(), 
                tufa_common::repositories_types::tufa_server::mongo_integration::mongo_insert_data::MongoInsertDataErrorUnnamed::MongoInsertDocsInEmptyCollection(*e)
            ));
        }
        None
    })
    .collect::<std::collections::HashMap<std::string::String, tufa_common::repositories_types::tufa_server::mongo_integration::mongo_insert_data::MongoInsertDataErrorUnnamed>>();
    if !error_hashmap.is_empty() {
        return Err(Box::new(
            tufa_common::repositories_types::tufa_server::mongo_integration::mongo_insert_data::MongoInsertDataErrorNamed::Errors {
