...
async fn get_provider_link_parts_from_mongo(
    ...
    let result_getting_provider_link_parts = ProviderKind::mongo_get_provider_link_parts_as_bson_string(provider_kind).await;
    match result_getting_provider_link_parts {
        Ok(provider_link_parts) => {
            ...
            vec_of_link_parts_hashmap_under_arc_handle_locked
                .insert(ProviderKind::get_string_name(provider_kind), provider_link_parts);
        }
        Err(e) => {
