...
pub async fn login(
    ...
    match tufa_common::repositories_types::tufa_server::authentication::validate_credentials(credentials, &pool).await {
        Ok(user_id) => {
        ...
        Err(e) => {
            let e = match e {
                tufa_common::repositories_types::tufa_server::authentication::AuthError::InvalidCredentials(_) => LoginError::AuthError(e.into()),
                tufa_common::repositories_types::tufa_server::authentication::AuthError::UnexpectedError(_) => LoginError::UnexpectedError(e.into()),
            };
