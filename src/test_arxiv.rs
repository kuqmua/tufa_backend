...
#[tokio::main]
pub async fn async_test_function(vec_of_links: Vec<&str>) -> HashMap<String, Vec<ArxivPost>>{
    ...
    println!(
            "async_test_function time fetching (in seconds) = {} ",
    ...
    println!(" after loop ");
    //println!("  vec_of_vec_of_strings {:#?}", vec_of_vec_of_strings[0]);
    ...
    for vec_member in vec_of_vec_of_strings {
        ...
        println!("vec_of_arxiv_posts.len() == 0");
            ...
            println!("vec_of_arxiv_posts[0].creators.len() == 0");
                ...
                println!("vec_of_arxiv_posts[0].creators[0].name {:#?}", vec_of_arxiv_post_second[0].creators[0].name);
            }
        }
        */
        //let str_wrapper: String = key_count.to_string();
        arxiv_pages_posts_hashmap_second.insert(key_count.to_string(), vec_of_arxiv_post_second);
        key_count +=1;
    }
    println!(" arxiv_pages_posts_hashmap_second {:#?}", arxiv_pages_posts_hashmap_second);
...
pub fn test_arxiv() ->  HashMap<String, Vec<ArxivPost>>
{
    ...
    //println!("{:#?}", vec_of_links);
    let vec_of_vec_of_strings = async_test_function(vec_of_links);
